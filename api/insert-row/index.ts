...
export default async function handler(req: any, res: any) {
  ...
  if (!host || !user || !database || !tableName || !row || typeof row !== 'object')
    return res.status(400).json({ error: 'Missing required fields: host,user,database,tableName,row' })

  if (!validateIdent(tableName)) return res.status(400).json({ error: 'Invalid table name' })

  const cols = Object.keys(row)
  if (cols.length === 0) return res.status(400).json({ error: 'Row must contain at least one column' })
  for (const c of cols) if (!validateIdent(c)) return res.status(400).json({ error: `Invalid column name: ${c}` })
